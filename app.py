...
with tab1:
    ...
    if st.session_state.selected_mosque:
        ...
        with col1:
            if st.button(f"✅ আলহামদুলিল্লাহ, পেয়েছি  : {df.at[idx, 'real']}", use_container_width=True):
                # ১. সবার আগে সেলিব্রেশন ও মেসেজ (Instant Feedback)
                st.balloons()
                message_place.success("ইতিহাস আপনাকে মনে রাখবে! 😉")
                
                try:
                    # ২. এরপর ব্যাকগ্রাউন্ডে ডাটা আপডেট
                    df.at[idx, 'real'] += 1
                    conn.update(data=df)
                    st.cache_data.clear()
                    ...
                    st.error("⚠️ নেটওয়ার্ক সমস্যার কারণে ভোটটি জমা দেওয়া যায়নি। আবার চেষ্টা করুন।")
